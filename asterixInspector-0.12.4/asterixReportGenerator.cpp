...
void AsterixReportGenerator::createReport(QTextStream& out, Type type, int fromBlock, int toBlock)
{
  if (g_asterixModel == 0 || fromBlock < 0 || fromBlock >= g_asterixModel->numBlocks())
    return;

  toBlock = qMin(toBlock, g_asterixModel->numBlocks());
